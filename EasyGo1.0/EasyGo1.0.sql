...
user_id           INT NOT NULL,           #用户ID(外键)
user_hobby_name   VARCHAR(10),                #爱好名称
FOREIGN KEY (user_id) REFERENCES USER(user_id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE house(
	...
	house_address	VARCHAR(100),	        #房源地址（？？）
...
#房屋设施表(一个房源多个设施)WIFI，电视，洗衣机，毛巾...
CREATE TABLE house_equipment(
house_equipment_no INT PRIMARY KEY AUTO_INCREMENT,#无意义
house_id INT,                                     #房源编号
equipment_wifi CHAR(2),                      #设施名称
equipment_linyu CHAR(2),                      #设施名称1
equipment_kongtiao CHAR(2),                      #设施名称2
FOREIGN KEY (house_id) REFERENCES house(house_id) ON DELETE CASCADE ON UPDATE CASCADE
);
 #订单表
CREATE TABLE orders
