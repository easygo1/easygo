...
import com.easygo.model.beans.gson.GsonAboutHouseDetail;import com.easygo.model.beans.gson.GsonUserInfoHobby;
import com.easygo.model.beans.gson.GsonAboutHouseManage;import com.easygo.model.beans.house.Equipment;
...
import com.easygo.model.dao.house.IHouseDateManageDAO;import com.easygo.model.dao.house.IHouseEquipmentDAO;
...
import com.easygo.model.impl.house.IHouseDateManageDAOImpl;import com.easygo.model.impl.house.IHouseEquipmentDAOImpl;
...
public class AppServlet extends HttpServlet {
	...
	// HouseDateManage的相关对象	HouseDateManage houseDateManage;	IHouseDateManageDAO houseDateManageDAO;	List<HouseDateManage> houseDateManageList;// 所有的房屋，包括下面两种	List<HouseDateManage> houseUserBuyList;// 用户预定的某个房屋的日期	List<HouseDateManage> houseNotList;// 房东设置的某个房屋的不可租的日期	// Assess的相关对象
...
*/			break;
		...
		case "getHouseDetailByID":
			...
			// GsonAboutHouse gsonAboutHouse = new GsonAboutHouse(houseList,
			// userList, housePhotoList, assessList, houseCollectList);
			GsonAboutHouseDetail gsonAboutHouseDetail = new GsonAboutHouseDetail(					house, housePhotoList, houseCollectList,					houseEquipmentList, user);
			gson = new Gson();
			result = gson.toJson(gsonAboutHouseDetail);			mPrintWriter.write(result);
			mPrintWriter.close();
			break;
		// 删除某个用户的收藏表的一条数据
		...
		case "addHouseCollect":
			...
			break;
		// 得到某个房屋的日期状态		case "getHouseDateByHouseId":			house_id = Integer.parseInt(request.getParameter("houseid"));			houseDateManageDAO = new IHouseDateManageDAOImpl();			// 得到该房屋的所有			/*			 * houseDateManageList = houseDateManageDAO			 * .selectAllDateById(house_id);			 */			houseUserBuyList = houseDateManageDAO					.selectAllDateById(house_id, 1);// 已租			houseNotList = houseDateManageDAO.selectAllDateById(house_id, 2);// 不可租			gson = new Gson();			GsonAboutHouseManage gsonAboutHouseManage = new GsonAboutHouseManage(					houseUserBuyList, houseNotList);			result = gson.toJson(gsonAboutHouseManage);			mPrintWriter.write(result);			mPrintWriter.close();			break;		// 用户修改时间后，进行数据库的管理		case "updateHouseDate":			// 先删除数据库中，房东曾经修改过的房屋的时间，然后再将新的时间加到数据库中			result = request.getParameter("houseDate");			houseDateManageDAO = new IHouseDateManageDAOImpl();			type = new TypeToken<List<HouseDateManage>>() {			}.getType();			gson = new Gson();			// APP传过来的数据			houseNotList = gson.fromJson(result, type);			// 将数据库中的房东设置的数据全部查出来，然后删除			houseDateManageList = houseDateManageDAO.selectAllDateById(					houseNotList.get(0).getHouse_id(), 2);			for (int i = 0; i < houseDateManageList.size(); i++) {				houseDateManage = houseDateManageList.get(i);				houseDateManageDAO.delHouseDate(houseDateManage);			}			for (HouseDateManage h : houseNotList) {				houseDateManageDAO.addHouseDate(h);			}			// mPrintWriter.write(result);			// mPrintWriter.close();			break;		// 根据用户的id查出此人的所有订单
